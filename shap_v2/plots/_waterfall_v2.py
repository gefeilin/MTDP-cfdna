...
def waterfall_v2(
    ...
    # Keep the bottom marker ticks but render labels via ax.text for stable placement.
    ax2.set_xticklabels(["", ""], fontsize=12, ha="left")
    ...
    # Keep the top marker ticks but render labels via ax.text for stable placement.
    ax3.set_xticklabels(["", ""], fontsize=12, ha="left")
    tick_labels = ax3.xaxis.get_majorticklabels()
    for _tl in tick_labels:
        _tl.set_color("#FF0000")
    ...
    # Place header/footer labels near their actual x-values (adaptive, not fixed corners).
    xtrans = blended_transform_factory(ax.transData, ax.transAxes)
    xspan = max(xmax - xmin, 1e-8)
    fx_frac = (fx - xmin) / xspan
    base_frac = (base_values - xmin) / xspan
    # Default: center labels on the marker value. Only switch near boundaries.
    fx_ha = "center"
    if fx_frac > 0.92:
        fx_ha = "right"
    elif fx_frac < 0.08:
    ...
    if base_frac > 0.92:
        ...
        base_ha = "left"
    fx_dx = -6 if fx_ha == "right" else (6 if fx_ha == "left" else 0)
    base_dx = -6 if base_ha == "right" else (6 if base_ha == "left" else 0)

    predicted_header = predicted_label if predicted_label is not None else f"{predicted_prefix} {target_label}"

    ax.annotate(
        f"{predicted_header}={format_value(fx, '%0.03f')}",
        xy=(fx, 1.0), xycoords=xtrans,
        xytext=(fx_dx, 8), textcoords="offset points",
        ha=fx_ha, va="bottom", fontsize=12, color="#FF0000",
        annotation_clip=False,
    )
    ax.annotate(
        f"{average_prefix} {target_label} = {format_value(base_values, '%0.03f')}",
        xy=(base_values, 0.0), xycoords=xtrans,
        xytext=(base_dx, -22), textcoords="offset points",
        ha=base_ha, va="top", fontsize=12, color="#FF0000",
        annotation_clip=False,
    )
...
def waterfall_legacy_v2(
    ...
    xspan = max(xmax - xmin, 1e-8)
    fx_frac = (fx - xmin) / xspan
    exp_frac = (expected_value - xmin) / xspan
    fx_ha = "center"
    if fx_frac > 0.92:
        fx_ha = "right"
    elif fx_frac < 0.08:
    ...
    if exp_frac > 0.92:
        ...
        exp_ha = "left"
    fx_dx = -6 if fx_ha == "right" else (6 if fx_ha == "left" else 0)
    exp_dx = -6 if exp_ha == "right" else (6 if exp_ha == "left" else 0)

    ax.annotate(
        f"{predicted_prefix} {target_label}={format_value(fx, '%0.03f')}",
        xy=(fx, 1.0), xycoords=xtrans,
        xytext=(fx_dx, 8), textcoords="offset points",
        ha=fx_ha, va="bottom", fontsize=12, color="#FF0000",
        annotation_clip=False,
    ...
    ax.annotate(
        f"{average_prefix} {target_label} = {format_value(expected_value, '%0.03f')}",
        xy=(expected_value, 0.0), xycoords=xtrans,
        xytext=(exp_dx, -22), textcoords="offset points",
        ha=exp_ha, va="top", fontsize=12, color="#FF0000",
        annotation_clip=False,
    )
